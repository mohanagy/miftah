...
import { asRemoteError } from "../src/upstream/remote-error.js";
import { UpstreamProcessManager } from "../src/upstream/upstream-process-manager.js";
import {
  startFakeRemoteUpstream,
  type FakeRemoteUpstream,
...
describe("remote upstream transports", () => {
  ...
  it("keeps streamable HTTP profile headers isolated while proxying capabilities", async () => {
    ...
    const config = validateConfig({
      ...
      profiles: {
        work: {
          headers: { authorization: "Bearer work-secret", "x-profile": "work" },
        },
        personal: {
          headers: {
    ...
    try {
      ...
      const workHeaders = upstream
        .requests()
        .filter((request) => request.headers["x-profile"] === "work");
      const personalHeaders = upstream
        .requests()
        .filter((request) => request.headers["x-profile"] === "personal");
      expect(workHeaders).not.toHaveLength(0);
      expect(personalHeaders).not.toHaveLength(0);
      expect(
        workHeaders.map((request) => request.headers.authorization),
      ).toEqual(expect.arrayContaining(["Bearer work-secret"]));
      expect(
        personalHeaders.map((request) => request.headers.authorization),
      ).toEqual(expect.arrayContaining(["Bearer personal-secret"]));
      expect(
        JSON.stringify(
  ...
  it("forces local cleanup when a Streamable HTTP session DELETE hangs", async () => {
    ...
    try {
      ...
      expect(
        await waitFor(() => upstream.hangingStreamableDeleteClosed()),
      ).toBe(true);
    } finally {
  ...
  it("uses the http compatibility alias for Streamable HTTP", async () => {
    ...
    try {
      ...
      expect(
        await session.callTool({ name: "whoami", arguments: {} }),
      ).toMatchObject({
        content: [{ type: "text", text: "work" }],
      });
    } finally {
  ...
  it("reports remote HTTP startup status without exposing an upstream response body", async () => {
    ...
    try {
      ...
      expect(error).toMatchObject({
        ...
        message:
          "UPSTREAM_HTTP_ERROR: streamable-http upstream for profile 'work' returned HTTP 401",
  ...
  it("ignores SDK v2 remote errors that do not carry a valid HTTP status", () => {
    expect(
      asRemoteError(
        "work",
        "streamable-http",
        new SdkHttpError(SdkErrorCode.NotConnected, "failed", { status: 42 }),
      ),
    ).toBeUndefined();
    expect(
      asRemoteError(
        "work",
        "sse",
        new SseError(undefined, "failed", {} as ErrorEvent),
      ),
    ).toBeUndefined();
  });
  ...
  it("reports remote MCP protocol errors without exposing an upstream error message", async () => {
    const upstream = await startFakeRemoteUpstream({
      callToolError: {
        code: -32603,
        message: "remote-server-secret must not escape",
      },
    });
    upstreams.push(upstream);
    const config = validateConfig({
  ...
  it("reports remote HTTP operation status without exposing an upstream response body", async () => {
    ...
    const [clientTransport, serverTransport] =
      InMemoryTransport.createLinkedPair();
    const client = new Client({
      name: "remote-operation-error-test",
      version: "1.0.0",
    });
