...
if (
  ...
  throw new Error(
    "Usage: node named-host-desktop-launcher.mjs <output-root> <node> <recorder> <miftah-cli> <fake-upstream> [features|cancellation]"
  );
}
if (!["features", "cancellation"].includes(attemptMode)) {
  throw new Error(`Unsupported named-host evidence attempt: ${attemptMode}`);
}
...
const subscribeCountPath = join(instanceDirectory, "resource-subscribe-count");
const unsubscribeCountPath = join(instanceDirectory, "resource-unsubscribe-count");
const subscribeStartedPath = join(instanceDirectory, "resource-subscribe-started");
const callToolStartedPath = join(instanceDirectory, "tool-call-started");
const cancelledPath = join(instanceDirectory, "upstream-cancelled");
  ...
  profiles: {
    work: {
      env: {
        ...
        TEST_CALL_TOOL_STARTED_PATH: callToolStartedPath,
        TEST_CALL_TOOL_DELAY_MS: attemptMode === "cancellation" ? "60000" : "0",
        ...
        TEST_RESOURCE_UPDATE_DELAY_MS: "250",
