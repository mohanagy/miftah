...
const clientRequestMethods = new Map();
const clientRequestProtocols = new Map();
...
const serverOperationMethods = new Set(["roots/list", "elicitation/create"]);
...
const makeRoomForClientRequest = (key) => {
  if (clientRequestMethods.has(key) || clientRequestMethods.size < maxPendingRequests) return;
  const oldestKey = clientRequestMethods.keys().next().value;
  clientRequestMethods.delete(oldestKey);
  clientRequestProtocols.delete(oldestKey);
};

const makeRoomForServerRequest = (key) => {
  if (serverRequestMethods.has(key) || serverRequestMethods.size < maxPendingRequests) return;
  const oldestKey = serverRequestMethods.keys().next().value;
  serverRequestMethods.delete(oldestKey);
};
...
const incrementNotification = (direction, method) => {
  const current = observations.notifications[direction][method] ?? 0;
  observations.notifications[direction][method] = current + 1;
};
...
const recordClientMessage = (message) => {
  ...
  if (typeof message.method !== "string") {
    ...
    const method = serverRequestMethods.get(key);
    if (method === undefined) return;
    serverRequestMethods.delete(key);
    if (serverOperationMethods.has(method)) {
  ...
  if (message.id !== undefined) {
    const key = requestKey(message.id);
    makeRoomForClientRequest(key);
    clientRequestMethods.set(key, message.method);
    if (clientOperationMethods.has(message.method)) {
      incrementOperation(message.method, "requests");
    }
  } else if (notificationMethods.has(message.method)) {
    incrementNotification("client", message.method);
  }
  ...
  if (typeof envelopeProtocol === "string") {
    ...
    if (message.id !== undefined) {
      clientRequestProtocols.set(requestKey(message.id), envelopeProtocol);
    }
...
const recordServerMessage = (message) => {
  ...
  if (typeof message.method === "string") {
    if (message.id !== undefined) {
      const key = requestKey(message.id);
      makeRoomForServerRequest(key);
      serverRequestMethods.set(key, message.method);
      if (serverOperationMethods.has(message.method)) {
        incrementOperation(message.method, "requests");
      }
    } else if (notificationMethods.has(message.method)) {
      incrementNotification("server", message.method);
    }
    return;
  }
  if (message.id === undefined) return;

  const method = clientRequestMethods.get(requestKey(message.id));
  if (method === undefined) return;
  clientRequestMethods.delete(requestKey(message.id));
  const requestProtocol = clientRequestProtocols.get(requestKey(message.id));
  clientRequestProtocols.delete(requestKey(message.id));

  if (clientOperationMethods.has(method)) {
    incrementOperation(method, message.error === undefined ? "success" : "error");
  }
