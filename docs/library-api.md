...
| MCP protocol eras | Modern `2026-07-28` and the documented initialized legacy era follow the [MCP protocol and client compatibility](mcp-compatibility.md) contract. Removing an era, transport, or retained deprecated behavior requires a separate evidence-backed issue and the major-release/remediation rule. |
