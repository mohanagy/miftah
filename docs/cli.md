...
`miftah dashboard` opens the optional browser-local Console and prints its exact URL, resolved first-run location, and one-use bootstrap code. Without `--config`, it inspects only direct, safe JSON configs in `~/.config/miftah` and requires the operator to choose one; it never scans client settings, process arguments, or arbitrary directories. A missing catalog permits strict first-run native-OAuth setup at `~/.config/miftah/miftah.json`; an existing file is never silently replaced. `miftah dashboard --config <file>` stays authoritative for one exact configuration and skips catalog discovery. `--no-open` leaves browser launch to the operator while keeping the same foreground server.
