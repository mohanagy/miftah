...
| MCP session IDs, approval bearers, profile locks, and leases | These capabilities can alter which operation is allowed or selected. | Connection binding, bounded lifetimes, keyed digests for approval bearers, and exact-operation binding. |
...
| **Misleading upstream identity or response data** is treated as proof of authorization or account scope. | Identity verification is opt-in, process-only, bounded, and keeps allowlisted fingerprints plus stable error codes; remote error bodies and raw identity output are not retained. | Identity verification is not authentication, credential validity, provider authentication, account authorization, or scope validation. It does not make a compromised upstream trustworthy. |
