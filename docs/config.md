...
Remote authentication can use explicit `headers`/secret references documented by the upstream. Version 3 can instead bind one strict non-secret `oauth.connections` record to an exact HTTPS Streamable HTTP profile/upstream target. The connection selects `pre-registered:<client-id>`, `client-id-metadata:<https-url>`, or `dynamic` registration; discovery must advertise the selected capability plus authorization code, PKCE `S256`, and the RFC 9207 issuer response parameter. Token, refresh-token, client-secret, callback, and same-target Authorization-header fields remain forbidden in configuration. Unknown OAuth keys and unsupported registration modes are rejected instead of being silently accepted. See [OAuth support](oauth-support.md) for a complete example, OS-vault behavior, and the provider/local-STDIO boundary.
