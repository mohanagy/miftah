...
Plugin ABI version `"1"` remains supported throughout the Miftah 1.x release line. Removal requires a new major release, an **Unreleased** changelog entry, and a migration guide that names the replacement ABI. The host rejects an unsupported ABI during preflight with `PLUGIN_API_INCOMPATIBLE`, before it serves MCP traffic. The package-root [compatibility policy](library-api.md#compatibility-policy) separately covers the stable subpath and management-tool names.
