...
| --- | --- | --- |
| `generic` | `npx --yes @modelcontextprotocol/server-everything@2026.7.4 stdio` | This is the MCP reference/test server, not a production-ready provider recommendation. Its npm metadata declares no upstream Node engine floor. An optional `--credential-env <name>` adds only a `${ENV_NAME}` reference. Unavailable on Windows because npm's `npx` runner requires a command shell. |
...
| `sentry` | `npx --yes @sentry/mcp-server@0.36.0 --skills=inspect`; generated environment reference: `SENTRY_ACCESS_TOKEN` | The upstream package requires Node.js `>=20`. Use least-privilege Sentry token scopes. `--skills=inspect` filters CLI skills; it is not provider-token authorization and is not a read-only flag or preset. Miftah local policy does not reduce provider token permissions. Unavailable on Windows because npm's `npx` runner requires a command shell. |
| `google-search-console` | Noninteractive one-account setup requires `--oauth-client-secrets-file <absolute-path>`; optional `--expected-account-id <opaque-id>` and `--identity-probe-tool <tool>` declare bounded account identity. Guided `miftah setup` supports one or more named accounts. Generated command: `uvx mcp-search-console@0.3.2`; generated environment: `GSC_OAUTH_CLIENT_SECRETS_FILE` and a distinct `GSC_CONFIG_DIR` per generated configuration file and profile. | Python `>=3.11` and `uvx` are required. OAuth, browser handoff, token cache, reauthentication, and revocation remain upstream/manual-owned. The pin does not currently expose the preferred identity probe, so identity is actionable but unsupported and protected risky operations fail closed; property access is not identity proof. Miftah applies a read-only policy, does not set `GSC_ALLOW_DESTRUCTIVE`, and never reads the upstream cache. See the [provider-adapter contract](provider-adapters.md#google-search-console-pilot). |
| `generic-npx` | Required: `--npm-package <exact-package@x.y.z>`; optional `--credential-env <name>` | Only an exact npm package SemVer is accepted. The selected external package, not Miftah, declares its own Node requirement. Unavailable on Windows because npm's `npx` runner requires a command shell. |
...
miftah init [name] \
  ...
  [--expected-account-id <opaque-id>] [--identity-probe-tool <tool>] \
...
`--interactive` is available only when both input and output are real TTYs. EOF or Ctrl-C cancels before the configuration write. The wizard asks only for a name, an outcome-first source answer (`remote` or `local`) or catalog connector name, safe preset metadata (variable names, URLs, header metadata, pins), output location, and client selection. Multi-profile GitHub and Google Search Console setup also asks whether active switches are temporary `process` state or durable `workspace` state. The equivalent scripted option is `--active-profile-lifetime <process|workspace>`. For `local-stdio`, the wizard asks for one literal argv item at a time and gives a count-only review summary before explicit acknowledgement. It never asks for or echoes a secret value.

`miftah setup [name]` is the dedicated guided command. It uses that same TTY-only wizard and safe preset options, with interactivity required by the command instead of supplied as `--interactive`. Its first step lists five numbered starting points: a known connector or pinned package, remote HTTPS endpoint, local executable, remote MCP with browser sign-in, or existing client entry. Enter a number or the matching name. The second step confirms the selected path; enter `back` there to return to the starting-point list before connection details are collected, or `cancel` to exit before a configuration is written. The plain remote and import paths do not discover OAuth or call the upstream. Keep `miftah setup --native-oauth` for a scripted or repeatable standards-based OAuth setup.

For one-account noninteractive `google-search-console` setup, `--oauth-client-secrets-file` is required and is written only to the generated profile environment. Optional `--expected-account-id` records a non-email opaque expected identifier; `--identity-probe-tool` is valid only with it. Guided `miftah setup` asks for one or more named accounts, their client-secrets paths, optional expected IDs, an explicit default, and—when multiple accounts are present—an active-profile lifetime. Multi-account identity is all-or-none and IDs must be distinct. `process` means a fresh Miftah process starts from the configured default; `workspace` writes schema-valid durable state and restores the last switch for this configuration. It also generates a separate upstream-owned `GSC_CONFIG_DIR` for each generated configuration file and profile. To add another account later, use `miftah setup --add-profile --config <file>` with a new `--profile` and `--oauth-client-secrets-file`; add `--expected-account-id` when the existing profiles use identity. The selected configuration must remain wholly within the reviewed adapter boundary and every existing state directory must already be distinct. The pinned upstream does not currently expose the preferred identity probe, and a property list is not identity evidence. The safe adapter summary printed by `init` does not echo a configured client-secrets path. The printed client snippets use absolute paths to the Node executable and compiled Miftah CLI so GUI clients do not depend on `PATH`. Regenerate them after moving or upgrading Miftah, or after changing the configuration path. Copy generated JSON as JSON; do not hand-edit the command into a shell string.
