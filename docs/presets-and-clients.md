...
`miftah setup [name]` is the dedicated guided command. It uses that same TTY-only wizard and safe preset options, with interactivity required by the command instead of supplied as `--interactive`. At its source question, `remote` selects the strict `streamable-http` path and `local` selects `local-stdio`; catalog names remain accepted. The plain remote path does not discover OAuth or call the upstream, so use `miftah setup --native-oauth` only for a remote server that advertises standards-based OAuth.
