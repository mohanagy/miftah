...
- **Current evidence:** `source-test` in `tests/mcp-v2-serving.test.ts` negotiates `2025-11-25`, exposes tools, distinguishes legacy subscription and cache behavior from the modern STDIO path, and forwards list changes plus cancellation through the SDK v2 initialized boundary. The corrected exact published-v1.1.2 `packaged-test` below drives the installed CLI process and proves initialized Roots routing and refresh, resource subscribe/update/unsubscribe, active-profile list changes, cancellation, terminal audit outcome, and cleanup with the official client. The exact published-v1.1.3 Codex CLI 0.148.0 `named-host-runtime` record negotiates initialized `2025-06-18`, lists tools once, calls one tool once, records successful audits, and shuts down the deterministic upstream. The exact published-v1.1.3 Claude Desktop deep record below observes initialized `2025-11-25`, one successful companion-session Roots request, all three primary-session list-change notifications, and host-native approval continuing to a successful fake tool result. Its stop-control attempt did not emit downstream MCP cancellation or a `cancelled` audit terminal.
...
- exact published-v1.1.3 named-host evidence proves one initialized Codex CLI list/call exchange, one modern Claude Code prompt/resource/tool discovery plus call exchange, and initialized Claude Desktop discovery, native approval continuation, one companion Roots request, and all three primary list-change notifications;
