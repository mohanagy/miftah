...
- [#122](https://github.com/mohanagy/miftah/issues/122) Windows OAuth transaction locking now acquires an exclusive, crash-released named pipe instead of binding inside the operating system's default ephemeral TCP range, preventing unrelated outbound connections from making the local credential store unavailable. It still probes the legacy canonical TCP endpoint before and after acquisition for rolling-upgrade coordination, while POSIX coordination remains unchanged.
