...
- [#420](https://github.com/mohanagy/miftah/issues/420) Prepared the compatible v1.1.3 patch release to publish the legacy-compatibility evidence ledger and exact-package initialized STDIO and Streamable HTTP harnesses for the continuing retirement gate [#388](https://github.com/mohanagy/miftah/issues/388), and advanced the build-time `nanoid` override from 3.3.17 to 3.3.18 to resolve GHSA-2v37-7h3g-55p8. Publication remains gated on exact `development`-to-`main` promotion and protected OIDC trusted publishing, a zero-known-vulnerability audit, registry provenance, a fresh install, and package-signature verification; this release does not authorize removal of any legacy behavior.
- [#416](https://github.com/mohanagy/miftah/issues/416) Added exact-package initialized Streamable HTTP evidence with the official legacy reference client. The deterministic consumer launches the clean-installed CLI, receives a real `Mcp-Session-Id`, completes one redacted form-elicitation approval with a successful terminal audit result, forwards one cancellation to the selected upstream with one `cancelled` / `REQUEST_CANCELLED` terminal, explicitly terminates the protocol session, proves its retained ID returns HTTP 404, and proves retained-upstream cleanup. This remains reference-client evidence only and does not establish named-host usage, migration, rollback, or retirement approval.
- [#413](https://github.com/mohanagy/miftah/issues/413) Corrected the exact-package legacy STDIO evidence harness: list-change signals now come from the active catalog profile, and the cancellable call must create a fresh start marker before abort. The corrected fixture proves that published v1.1.2 delivers all three advertised list changes, forwards cancellation exactly once, and records one terminal `cancelled` / `REQUEST_CANCELLED` audit event; no runtime defect or runtime change was required.
...
- [#368](https://github.com/mohanagy/miftah/issues/368) Added an evidence-backed MCP protocol and client compatibility contract covering modern `2026-07-28` and initialized `2025-11-25` STDIO/Streamable HTTP paths, deprecated core features, extensions, notifications, actionable fallback errors, and the distinction between stateless protocol requests and stateful Miftah application data. A pinned MCP Inspector 2.1.0 gate now drives the installed tarball over STDIO and modern Streamable HTTP on Linux Node 22; named desktop-host claims remain explicitly unverified where no packaged runtime transcript exists. Any legacy retirement is deferred to the separate post-v1.1 major-version evidence gate [#388](https://github.com/mohanagy/miftah/issues/388).
...
- [#393](https://github.com/mohanagy/miftah/issues/393) Prepared the compatible v1.1.0 minor release for the MCP 2026-07-28, stateless profile-context, MRTR, OAuth, gateway, and dual-era compatibility work. Runtime interoperability claims remain limited to the locked MCP TypeScript client and pinned MCP Inspector operations; named desktop-host rows remain configuration-shape evidence until an exact packaged-host transcript exists. Publication remains gated on exact `development`-to-`main` promotion and protected OIDC trusted publishing, registry provenance, a fresh install, and package-signature verification.
